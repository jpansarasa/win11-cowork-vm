...
# config.env is tracked and holds portable defaults/placeholders. Site-specific
# values (a WAN IP, an internal hostname) belong in config.local.env, which is
# gitignored — so a public clone can never leak them and `git status` stays clean.
# Sourced second, so it wins.
load_config() {

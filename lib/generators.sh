...
# --- WAN-tracking DNS refresh -------------------------------------------------
# A DNS_OVERRIDES entry pins an IP. When that IP is a dynamic WAN address (DDNS),
# it goes stale on renumber and the guest silently cannot reach the service. These
# generate a host-side timer that re-points the libvirt <dns><host> entries named
# in DNS_WAN_HOSTS at the current public IP. Pure: they only print text.

gen_wandns_script() {
  cat <<EOF
#!/bin/sh
# cowork-wandns - keep libvirt <dns><host> entries pointed at the current WAN IP.
# GENERATED by gen_wandns_script (scripts/36-wandns.sh) - do not edit in place.
set -u
NET='${NET_NAME}'
HOSTS='${DNS_WAN_HOSTS}'
LOOKUPS='${WAN_IP_LOOKUPS}'

[ -n "\$HOSTS" ] || exit 0

# First lookup that returns a bare IPv4 wins. A wrong answer here would point the
# guest at someone else's address, so validate shape before trusting it.
wan=''
for u in \$LOOKUPS; do
  v=\$(curl -fsS --max-time 10 "\$u" 2>/dev/null | tr -d '[:space:]')
  case "\$v" in
    ''|*[!0-9.]*) continue ;;
  esac
  case "\$v" in
...
if [ -z "\$wan" ]; then
  echo "cowork-wandns: could not determine WAN IP (tried: \$LOOKUPS)" >&2
  exit 1
fi

rc=0
for h in \$HOSTS; do
  # Current IP libvirt has for this hostname (quote-agnostic, no XML tooling).
  cur=\$(virsh net-dumpxml "\$NET" 2>/dev/null | awk -v h="\$h" '
    /<host ip=/ {
      s = \$0
      i = index(s, "ip=")
      q = substr(s, i + 3, 1)
      rest = substr(s, i + 4)
      j = index(rest, q)
      ip = substr(rest, 1, j - 1)
    }
    \$0 ~ ("<hostname>" h "</hostname>") { print ip; exit }
  ')

  [ "\$cur" = "\$wan" ] && continue

  if [ -n "\$cur" ]; then
    virsh net-update "\$NET" delete dns-host \\
      "<host ip='\$cur'><hostname>\$h</hostname></host>" --live --config >/dev/null 2>&1 || true
  fi
  if virsh net-update "\$NET" add dns-host \\
      "<host ip='\$wan'><hostname>\$h</hostname></host>" --live --config >/dev/null 2>&1; then
    echo "cowork-wandns: \$h \${cur:-<unset>} -> \$wan"
  else
    echo "cowork-wandns: FAILED to update \$h to \$wan" >&2
    rc=1
  fi
done
exit \$rc
...
gen_wandns_service() {
  cat <<EOF
[Unit]
Description=Re-point Cowork guest DNS overrides at the current WAN IP (dynamic address)
After=libvirtd.service network-online.target
Wants=libvirtd.service network-online.target

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/cowork-wandns
EOF
}
...
gen_wandns_timer() {
  cat <<EOF
[Unit]
Description=Periodically refresh Cowork guest DNS overrides against the WAN IP

...
# --- host-mediated notification relay ----------------------------------------
# The guest cannot reach ntfy (the cage drops guest->LAN) and holds no token. It
# spools requests; the host drains them over qemu-guest-agent and publishes from
# here, where ntfy has always been reachable. Pure: these only print text.

gen_relay_env() {
  cat <<EOF
# Generated by scripts/37-notify-relay.sh - do not edit in place.
VM_NAME=${VM_NAME}
NTFY_URL=${NTFY_URL}
NTFY_TOKEN_FILE=${NTFY_TOKEN_FILE}
OUTBOX=${NOTIFY_OUTBOX}
EOF
}
...
gen_relay_service() {
  cat <<EOF
[Unit]
Description=Publish the Cowork guest's queued notifications to ntfy (guest cannot reach it directly)
After=libvirtd.service network-online.target
Wants=libvirtd.service

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/cowork-notify-relay
EOF
}
...
gen_relay_timer() {
  cat <<EOF
[Unit]
Description=Periodically drain the Cowork guest notification outbox

[Timer]
OnBootSec=2min
OnUnitActiveSec=${NOTIFY_POLL_INTERVAL}
Persistent=true
